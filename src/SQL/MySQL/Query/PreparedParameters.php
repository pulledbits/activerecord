...
class PreparedParameters implements \Countable
{
    ...
    public function __construct($parameters)
    {
        ...
        foreach ($parameters as $localColumn => $value) {
            $this->preparedParameters[] = [self::PP_COLUMN => $localColumn, self::PP_VALUE => $value, self::PP_PARAM => ":" . uniqid()];
        }
    }
    ...
    {
        return array_map(function (array $preparedParameter) use ($type) {
            return $preparedParameter[$type];
        }, $this->preparedParameters);
    }

    public function extractParametersSQL()
    {
        return array_map(function ($preparedParameter) {
            return $preparedParameter[self::PP_COLUMN] . " = " . $preparedParameter[self::PP_PARAM];
        }, $this->preparedParameters);
    }
